 ...
 *
 */
public class XCalendarHourWidget extends AbstractXCalendarWidget {
	//
	 ...
	 * 
	 */
	public XCalendarHourWidget(XCalendar popup, int col, int row) {
	...
	public Pair<Point> locate(int x, int y, int w, int m) {
		int unit = w / 4; int height = unit / 2 + unit / 5;
	...
	protected boolean onMouseUp(final XCalendarMouseUpEvent event) {
		...
		v.setHour(parseInt(theme.getHourTheme()[row][col]));
